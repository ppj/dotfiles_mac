---
name: add-epic
description: Add a new epic to a project in Linear (represented as a Milestone). Use when the user asks to add/create a new epic for a project, e.g. "add an epic to shruti called Auth Rework", "new epic for naadshala: onboarding flow". Works from any project's own working directory, not just from the journal vault.
---
...
- Milestone = epic.
- Issue = task (see the `add-task` skill).
...
3. Create the milestone (`save_milestone`, `project: <name>`, `name: <epic name>`, and
   `description` if the user gave one).
4. If the user gave initial tasks, create each as an issue (`save_issue`,
   `team: "Prasanna Joshi"`, `project: <name>`, `milestone: <epic name>`,
   `title: <task>`).
